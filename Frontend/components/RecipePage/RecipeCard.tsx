...
export default function RecipeCard({ recipe, showCookButton = true }: RecipeCardProps) {
  ...
  const placeholderImage = "https://via.placeholder.com/150?text=Recipe";
  const imageUrl = recipe.image ? recipe.image : placeholderImage;
  ...
  return (
    <TouchableOpacity style={styles.card} onPress={handlePress}>
      ...
      <View style={styles.footer}>
        ...
        {showCookButton && (
          <TouchableOpacity
            style={[styles.cookButton, isCooking && styles.cookingButton]}
            onPress={handleCook}
...
const styles = StyleSheet.create({
  ...
  badge: {
    ...
    paddingVertical: 4,
    paddingHorizontal: 8,
    marginRight: 8, // Fixes the missing `gap`
  },
  badgeText: {
    fontSize: 12,
    color: COLORS.text,
